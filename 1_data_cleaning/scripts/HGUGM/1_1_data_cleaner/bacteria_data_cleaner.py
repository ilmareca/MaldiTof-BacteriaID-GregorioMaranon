...
def setup_logging(year):
    ...
    log_directory = f'/export/usuarios01/ilmareca/github/MaldiTof-BacteriaID-GregorioMaranon/1_data_cleaning/logs/HGUGM/{year}'
    ...
    xml_found_log_handler = logging.FileHandler(os.path.join(log_directory, f'xml_found_log_{year}_{timestamp}.log'))
    xml_found_log.addHandler(xml_found_log_handler)

    analyte_validation_log = logging.getLogger('analyte_validation_log')
    analyte_validation_log_handler = logging.FileHandler(os.path.join(log_directory, f'analyte_validation_log_{year}_{timestamp}.log'))
    ...
    analyte_zip_found_log_handler = logging.FileHandler(os.path.join(log_directory, f'analyte_zip_found_log_{year}_{timestamp}.log'))
    analyte_zip_found_log.addHandler(analyte_zip_found_log_handler)
    
    directory_move_log = logging.getLogger('directory_move_log')
    directory_move_log_handler = logging.FileHandler(os.path.join(log_directory, f'directory_move_log_{year}_{timestamp}.log'))
    directory_move_log.addHandler(directory_move_log_handler)

    # Logger for recording errors that occur during processing
    error_log = logging.getLogger('error_log')
    error_log_handler = logging.FileHandler(os.path.join(log_directory, f'error_log_{year}_{timestamp}.log'))
