...
output_dir = '/export/usuarios01/ilmareca/github/MaldiTof-BacteriaID-GregorioMaranon/4_feature_extraction/scripts/lle/figures/minmax_variance'
score_output_file = os.path.join('/export/usuarios01/ilmareca/github/MaldiTof-BacteriaID-GregorioMaranon/4_feature_extraction/scripts/lle/outputs', 'lle_minmax_variance_grid.csv')
os.makedirs('/export/usuarios01/ilmareca/github/MaldiTof-BacteriaID-GregorioMaranon/4_feature_extraction/scripts/lle/outputs', exist_ok=True)
...
# Define the grid of hyperparameters for LLE and feature selection
n_neighbors_list = [5, 10, 15, 30]  # Different neighborhood sizes for LLE
...
# Perform LLE for each antibiotic and hyperparameter combination
for antibiotic in antibiotics:
    ...
    for threshold in variance_threshold_list:
        ...
        for n_neighbors in n_neighbors_list:
            # Perform LLE with the current hyperparameter combination
            ...
            if len(set(labels)) > 1:  # Silhouette score needs at least 2 classes
                silhouette = silhouette_score(X_lle, labels)
            else:
                silhouette = float('nan')  # Assign NaN if only one class exists

            # Create a DataFrame for visualization
            df_lle = pd.DataFrame(X_lle, columns=['LLE1', 'LLE2'])
            df_lle['label'] = df_filtered[antibiotic].values
            df_lle['color'] = df_lle['label'].map(color_map)

            # Plot
            plt.figure(figsize=(10, 7))
            plt.scatter(df_lle['LLE1'], df_lle['LLE2'], c=df_lle['color'], alpha=0.7, s=15, edgecolor='k', linewidth=0.3)

            # Titles and labels
            plt.title(f'LLE ({antibiotic})\nScore: {silhouette:.3f}, n_neighbors={n_neighbors}, variance={threshold}')
            plt.xlabel('LLE1')
            plt.ylabel('LLE2')

            # Add legend
            legend_labels = df_lle['label'].unique()
            ...
            file_name = f'lle_{safe_antibiotic_name}_n{n_neighbors}_var{threshold}.png'
...
results_df = pd.DataFrame(results, columns=['Antibiotic', 'n_neighbors', 'variance_threshold', 'silhouette_score', 'file_name'])
