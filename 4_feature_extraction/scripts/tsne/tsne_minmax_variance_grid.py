...
output_dir = '/export/usuarios01/ilmareca/github/MaldiTof-BacteriaID-GregorioMaranon/4_feature_extraction/scripts/tsne/figures/minmax_variance'
score_output_file = os.path.join('/export/usuarios01/ilmareca/github/MaldiTof-BacteriaID-GregorioMaranon/4_feature_extraction/scripts/tsne/outputs', 'tsne_minmax_variance_grid.csv')
    ...
    for threshold in variance_threshold_list:
        ...
        for perplexity in perplexity_list:
            for learning_rate in learning_rate_list:
                ...
                # Titles and labels
                plt.title(f't-SNE ({antibiotic})\nScore: {silhouette:.3f}, perplexity={perplexity}, learning_rate={learning_rate}, variance={threshold}')
                plt.xlabel('TSNE1')
                plt.ylabel('TSNE2')
                ...
                file_name = f'tsne_{safe_antibiotic_name}_perp{perplexity}_lr{learning_rate}_var{threshold}.png'
...
results_df = pd.DataFrame(results, columns=['Antibiotic', 'perplexity', 'learning_rate', 'variance_threshold', 'silhouette_score', 'file_name'])
