...
output_dir = '/export/usuarios01/ilmareca/github/MaldiTof-BacteriaID-GregorioMaranon/4_feature_extraction/scripts/pca/figures/minmax_corr'
score_output_file = os.path.join('/export/usuarios01/ilmareca/github/MaldiTof-BacteriaID-GregorioMaranon/4_feature_extraction/scripts/pca/outputs', 'pca_minmax_corr_grid.csv')
os.makedirs('/export/usuarios01/ilmareca/github/MaldiTof-BacteriaID-GregorioMaranon/4_feature_extraction/scripts/pca/outputs', exist_ok=True)
...
# Perform PCA for each antibiotic and hyperparameter combination
for antibiotic in antibiotics:
    ...
    for threshold in threshold_list:
        ...
        for n_components in n_components_list:
            ...
            # Create a DataFrame for visualization (only if 2 components)
            if n_components == 2:
                df_pca = pd.DataFrame(X_pca, columns=['PCA1', 'PCA2'])
                df_pca['label'] = df_filtered[antibiotic].values
                df_pca['color'] = df_pca['label'].map(color_map)

                # Plot
                plt.figure(figsize=(10, 7))
                plt.scatter(df_pca['PCA1'], df_pca['PCA2'], c=df_pca['color'], alpha=0.7, s=15, edgecolor='k', linewidth=0.3)

                # Titles and labels
                plt.title(f'PCA ({antibiotic})\nScore: {silhouette:.3f}, n_components={n_components}, threshold={threshold}')
                plt.xlabel('PCA1')
                plt.ylabel('PCA2')

                # Add legend
                legend_labels = df_pca['label'].unique()
                ...
                file_name = f'pca_{safe_antibiotic_name}_n{n_components}_thresh{threshold}.png'
            ...
            # Store the results
            results.append([antibiotic, n_components, threshold, silhouette])

# Save the results to a CSV file
results_df = pd.DataFrame(results, columns=['Antibiotic', 'n_components', 'threshold', 'silhouette_score'])
