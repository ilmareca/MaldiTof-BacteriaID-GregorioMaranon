...
grid_search_results_file = os.path.join('/export/usuarios01/ilmareca/github/MaldiTof-BacteriaID-GregorioMaranon/5_modeling/scripts/random_forest/outputs', 'raw_isomap_random_forest_gridsearch.csv')
...
# Isomap components to test
isomap_components = [10, 50, 100, 500, 1000, 2000, 3000, 4000, 5000]

# Store results
all_results = []

for n_components in isomap_components:
    print(f"\nApplying Isomap with {n_components} components...")
    ...
    grid_search.fit(X_train_isomap, y_train)

    # Get the best model
    best_model = grid_search.best_estimator_

    # Make predictions
    y_pred = best_model.predict(X_test_isomap)
    ...
    # Store results
    results = {
        'Isomap_Components': n_components,
        'Accuracy': accuracy,
...
print(f"Grid Search results with Isomap saved to {grid_search_results_file}")
