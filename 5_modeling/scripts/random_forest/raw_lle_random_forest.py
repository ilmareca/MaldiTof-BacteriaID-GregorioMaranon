...
grid_search_results_file = os.path.join('/export/usuarios01/ilmareca/github/MaldiTof-BacteriaID-GregorioMaranon/5_modeling/scripts/random_forest/outputs', 'raw_lle_random_forest_gridsearch.csv')
...
# LLE components to test
lle_components = [2, 3, 5, 10]

# Store results
all_results = []

for n_components in lle_components:
    print(f"\nApplying LLE with {n_components} components...")
    lle = LocallyLinearEmbedding(n_neighbors=15, n_components=n_components, random_state=42)
    ...
    grid_search.fit(X_train_lle, y_train)

    # Get the best model
    best_model = grid_search.best_estimator_

    # Make predictions
    y_pred = best_model.predict(X_test_lle)
...
print(f"Grid Search results with LLE saved to {grid_search_results_file}")
