...
grid_search_results_file = os.path.join('/export/usuarios01/ilmareca/github/MaldiTof-BacteriaID-GregorioMaranon/5_modeling/scripts/random_forest/outputs', 'raw_tsne_random_forest_gridsearch.csv')
...
for n_components in tsne_components:
    print(f"\nApplying t-SNE with {n_components} components...")
    tsne = TSNE(n_components=n_components, random_state=42)
    X_train_tsne = tsne.fit_transform(X_train)
    X_test_tsne = tsne.fit_transform(X_test)  # t-SNE does not have transform, so use fit_transform
...
print(f"Grid Search results with t-SNE saved to {grid_search_results_file}")
