...
grid_search_results_file = os.path.join('/export/usuarios01/ilmareca/github/MaldiTof-BacteriaID-GregorioMaranon/5_modeling/scripts/random_forest/outputs', 'raw_rfe_feature_selection_random_forest_gridsearch.csv')
...
rfe_selector = RFE(estimator=rf_for_rfe, n_features_to_select=100, step=10)  # Adjust features and step as needed
X_selected = rfe_selector.fit_transform(X_scaled, labels)

# Split the data into training and test sets
X_train, X_test, y_train, y_test = train_test_split(X_selected, labels, test_size=0.35, random_state=42)
...
print(f"Grid Search results with RFE feature selection saved to {grid_search_results_file}")