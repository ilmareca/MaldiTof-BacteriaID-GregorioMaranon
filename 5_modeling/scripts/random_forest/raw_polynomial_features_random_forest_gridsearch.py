...
grid_search_results_file = os.path.join('/export/usuarios01/ilmareca/github/MaldiTof-BacteriaID-GregorioMaranon/5_modeling/scripts/random_forest/outputs', 'raw_polynomial_features_random_forest_gridsearch.csv')
...
poly = PolynomialFeatures(degree=2, interaction_only=False, include_bias=False)  # Adjust degree if necessary
X_poly = poly.fit_transform(X_scaled)

# Split the data into training and test sets
X_train, X_test, y_train, y_test = train_test_split(X_poly, labels, test_size=0.35, random_state=42)
...
print(f"Grid Search results with polynomial feature expansion saved to {grid_search_results_file}")
