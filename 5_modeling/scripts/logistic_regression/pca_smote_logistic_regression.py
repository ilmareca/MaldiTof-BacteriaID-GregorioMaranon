...
grid_search_results_file = os.path.join('/export/usuarios01/ilmareca/github/MaldiTof-BacteriaID-GregorioMaranon/5_modeling/scripts/logistic_regression/outputs', 'smote_logistic_regression_result.csv')
...
pca = PCA(n_components=500)  # Adjust the number of components if needed
X_pca = pca.fit_transform(X_filtered)

# Split the data into training and test sets
X_train, X_test, y_train, y_test = train_test_split(X_pca, labels, test_size=0.35, random_state=42, stratify=labels)
...
# Define Logistic Regression model with specific hyperparameters
